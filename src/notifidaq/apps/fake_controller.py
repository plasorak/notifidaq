...
from notifidaq.models.controller_notification_pb2 import ControllerFoundChildren
import click
import logging
...
def main(bootstrap, instance_name, session_name, children_name):
    ...
    producer.notify(NotificationType.Controller_FoundChildren,ControllerFoundChildren() )
    ...
    producer.notify(
        NotificationType.Application_Started,ApplicationStarted()
    )
    producer.notify(
        NotificationType.Application_AdvertisedConnectivityService,ApplicationAdvertisedConnectivityService()
    )
